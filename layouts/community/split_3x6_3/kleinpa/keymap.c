...
void dance_right_layer_cycle(qk_tap_dance_state_t *state, void *user_data) {
    if (state->count - 1 < (sizeof(dance_right_layers) / sizeof(dance_right_layers[0]))) {
        layer_move(dance_right_layers[state->count - 1]);
    }
}
void dance_right_layer_reset(qk_tap_dance_state_t *state, void *user_data) { layer_clear(); }
enum {
    TD_LEFT_LAYER_CYCLE,
...
/* |        |   q    |   w    |   e    |   r    |   t    |   y    |   u    |   i    |   o    |   p    |        | */
    ...
    _______, KC_PSCR, KC_SLCK, KC_INS,  KC_F11,  KC_F12,  _______, KC_HOME, KC_PGDN, KC_PGUP, KC_END,  _______,
